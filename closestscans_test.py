...
class TestClosestScans(TestCase):
    def test_get_scans(self):
        ...
        cog = closestscans.get_scans('data', 'B99-999', 'FAKECOG_S')
        fdg = closestscans.get_scans('data', 'B99-999', 'FAKEFDG')
        assert_equal(cog, real_cog)
        assert_equal(fdg, real_fdg)

    def test_parse_date(self):
        d = closestscans.parse_date('2013-3-1')
    ...
    def test_date_delta(self): 
        ...
        assert_equal(closestscans.date_delta(d1, d2), 12)

    def test_find_closest(self):
        fdg = closestscans.get_scans('data', 'B99-999', 'FAKEFDG')
        closest = closestscans.find_closest(fdg, datetime(2012, 4, 4))
    ...
    def test_parse_excel(self):
        ...
        entries = closestscans.parse_excel('data/example.xls')
